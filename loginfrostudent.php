...
if (isset($_POST['register'])) {
    ...
    $check = "SELECT * FROM student WHERE username='$username' OR email='$email'";
    $check_result = mysqli_query($conn, $check);

    if (mysqli_num_rows($check_result) > 0) {
        $message = "User already exists! Please try logging in.";
    } else {
        $sql = "INSERT INTO student (fullname, email, username, password)
...
if (isset($_POST['update'])) {
    ...
    $sql = "SELECT * FROM student WHERE username='$username' AND password='$old_password'";
    $result = mysqli_query($conn, $sql);

    if (mysqli_num_rows($result) == 1) {
        $update_sql = "UPDATE student SET password='$new_password' WHERE username='$username'";
